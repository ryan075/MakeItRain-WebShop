...
if ( ! class_exists( 'Woo_Product_Slider_Import_Export' ) ) {
	...
	class Woo_Product_Slider_Import_Export {
		...
		public function export_shortcodes() {
			if ( ! wp_verify_nonce( $_POST['nonce'], 'spwps_options_nonce' ) ) {
				die();
			}
			$shortcode_ids = isset( $_POST['wpsp_ids'] ) ? $_POST['wpsp_ids'] : '';
		...
		public function import_shortcodes() {
			...
			if ( ! $data ) {
				wp_send_json_error(
					array(
						'message' => __( 'Nothing to import.', 'woo-product-slider-pro' ),
					),
					400
				);
			}
