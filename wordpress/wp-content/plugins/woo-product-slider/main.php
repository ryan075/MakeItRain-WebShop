	...
	class SP_WooCommerce_Product_Slider {
		...
		public function sp_wps_duplicate_shortcode() {
			...
			if ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] ) || ( isset( $_REQUEST['action'] ) && 'sp_wps_duplicate_shortcode' == $_REQUEST['action'] ) ) ) {
				wp_die( __( 'No shortcode to duplicate has been supplied!', 'woo-product-slider' ) );
			}

			/*
			 * Nonce verification
			 */
			if ( ! isset( $_GET['sp_wps_duplicate_nonce'] ) || ! wp_verify_nonce( $_GET['sp_wps_duplicate_nonce'], basename( __FILE__ ) ) ) {
				return;
			}
			...
			if ( isset( $post ) && $post != null ) {
				...
				wp_die( __( 'Shortcode creation failed, could not find original post: ', 'woo-product-slider' ) . $post_id );
			}
		}
		...
		*/
		public function sp_wps_duplicate_shortcode_link( $actions, $post ) {
			if ( current_user_can( 'edit_posts' ) && $post->post_type == 'sp_wps_shortcodes' ) {
